...
// pass, the approval record is cleared, and the FSM proceeds to documentation.
...
func runReview(ctx context.Context, o *run) error {
	...
	// falls through to the normal review loop.
	if o.tryAdoptApproval(ctx, plan) {
		return nil
	}
...
// It returns true when adoption succeeds: the record exists, the current branch
// HEAD matches the recorded SHA, and the verify gate (when the plan has a
...
// and returns nil for the FSM to proceed to documentation. On any mismatch
// (no record, different HEAD, red or erroring verify) it does nothing and
// returns false so the caller falls through to the normal review loop.
func (o *run) tryAdoptApproval(ctx context.Context, plan verifyPlan) bool {
	d := o.d
	...
	// the live path uses so the PR body and completion note are honest.
...
func (o *run) runSpecialists(ctx context.Context, authoritative bool) (string, error) {
	...
	results, err := harness.SpawnSubagents(ctx, d.Client, cfg.Workspace, d.Emit, specs,
		harness.SubagentOpts{
			...
			ExtraReadOnlyTools: reviewSubagentTools(cfg.CardID, cfg.Workspace, d.ReadRoots, d.SkillTool, d.ReadRootsLog),
...
// declared the panel gets exactly what it got before. cardID must match the
// identity the worker's own construction sites log with (Config.CardID) -
// rrLog's dedup key is (cardID, workspace, outcome), so a mismatched cardID
// (e.g. "") would never collapse with the run's earlier worker-path line for
// the same declaration. rrLog is the run's shared read-roots tracker
// (Deps.ReadRootsLog); nil is safe (logs without dedup) for callers with
// none to hand in.
func reviewSubagentTools(cardID, workspace string, roots []string, skill tools.Tool, rrLog *ReadRootsLog) []tools.Tool {
	...
	rrLog.Log(cardID, workspace, readTool.ReadRoots())
