...
- Do not create subtasks for release mechanics — tagging, versioning,
  pushing, publishing, deploying. Git belongs to the orchestrator (one
  commit per subtask); releases belong to humans or downstream automation.
  If the parent card's acceptance mentions a release step, note it as
  out-of-scope for the plan rather than decomposing it into a subtask.
