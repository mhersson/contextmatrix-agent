...
# -------------------------------------------------
# The agent is installed as a *user* unit under ~/.config/systemd/user.
# Rationale:
#   - The agent needs to talk to the user's Docker socket and uses
...
SERVICE_NAME="contextmatrix-agent"
SERVICE_FILE="${HOME}/.config/systemd/user/${SERVICE_NAME}.service"
BINARY="${SCRIPT_DIR}/contextmatrix-agent"
CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/contextmatrix-agent/serve.yaml"
...
cmd_verify() {
    ...
    if command -v systemd-analyze >/dev/null 2>&1; then
        local tmp
        tmp="$(mktemp --tmpdir "cm-agent-verify-XXXXXX.service")"
